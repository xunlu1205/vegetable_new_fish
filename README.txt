...
| 函式名稱                  | 功能說明                                  |
| --------------------- | ------------------------------------- |
| `setup()`             | 初始化所有腳位與硬體（馬達、紅外線、超音波、LED、蜂鳴器），只執行一次。 |
| `loop()`              | 主循環函式，根據狀態機控制車子動作，是程式的**主控中樞**。       |
| `readUltrasonic()`    | 讀取超音波感測器的距離，單位為公分，是觸發 STOP 狀態的依據。     |
| `moveForward()`       | 控制馬達讓車子前進。                            |
| `turnLeft()`          | 控制馬達讓車子左轉。                            |
| `turnRight()`         | 控制馬達讓車子右轉。                            |
| `stopCar()`           | 停止所有馬達，讓車子靜止不動。                       |
| `playHappyBirthday()` | 透過蜂鳴器播放生日快樂歌，是 STOP 狀態時的附加功能。         |
